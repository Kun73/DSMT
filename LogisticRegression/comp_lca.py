...
def com_run(dataset, graph_type, n, dstep_size, iid=True, cepoch=400, T=100, cstep_size=0.1, momentum=0.95):
    ...
    eta_w = (1 + eta_root) / 2
    beta = np.sqrt(eta_w)
    # beta = 1 - (1 - np.sqrt(eta_w)) / n**(1/2)


    dsmt_noLCA = dsmt_noLCA_run(theta_opt, F_opt, lr_0, beta, B, dstep_size, int(T), theta_0)
    ...
    # dsgdm = dsgdm_lca_run(theta_opt, F_opt, lr_0, B, dstep_size, int(T), theta_0)
    dsgd = dsgd_run(theta_opt, F_opt, lr_0, B, dstep_size, int(T), theta_0, False)
    edas = edas_run(theta_opt, F_opt, lr_0, B, dstep_size, int(T), theta_0)
    ...
    return sgd, sgdm, dsgd, dsgt, edas, dsgt_hb, dsmt, dsmt_noLCA
